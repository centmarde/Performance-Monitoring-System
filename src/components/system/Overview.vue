      ...
      <v-card>
        ...
        <v-card-text>
          <v-list v-if="assignedSubjects.length > 0" class="mt-2">
            <v-list-subheader class="font-weight-bold">Your Assigned Subjects:</v-list-subheader>
            <v-list-item v-for="subject in assignedSubjects" :key="subject.id" class="mb-2">
          ...
          <p class="mt-4">Contact your administrator to add or modify subjects assigned to you.</p>
          <p class="mt-2">Your administrator can assign you to teach different subjects based on your qualifications and the school's needs.</p>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" variant="text" @click="subjectsDialog = false">Close</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        const [subjectsCount, failingStudents, passingStudents, classRecordsCount] = await Promise.all([
